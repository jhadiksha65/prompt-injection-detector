...
EXP_DIR = "experiments/logistic_regression"
MODEL_PATH = f"{EXP_DIR}/lr_model.joblib"
...
def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - LOGISTIC REGRESSION EXPERIMENT")
    print("=" * 60)
    ...
    print(f"Logistic Regression training time: {train_time:.2f} seconds")
    ...
    # Evaluate all splits
    train_metrics = evaluate_model(y_train, y_train_probs, chosen_threshold, "Logistic Regression (Train - Diagnostic)", len(y_train), train_inf_time)
    val_metrics = evaluate_model(y_val, y_val_probs, chosen_threshold, "Logistic Regression (Validation - Tuned)", len(y_val), val_inf_time)
    test_metrics = evaluate_model(y_test, y_test_probs, chosen_threshold, "Logistic Regression (Test - Final Evaluation)", len(y_test), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  Logistic Regression")
    print(f"Configuration:              LogisticRegression(max_iter=1000, random_state=42), threshold={chosen_threshold:.2f}")
