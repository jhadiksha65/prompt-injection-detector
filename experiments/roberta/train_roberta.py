...
def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - ROBERTA EXPERIMENT")
    print("=" * 60)
    ...
    print(f"RoBERTa training completed in {train_time:.2f} seconds")
    
    # Restore best checkpoint
    model.load_state_dict({k: v.to(DEVICE) for k, v in best_model_state.items()})
    
    # Save the model
    torch.save(best_model_state, f"{EXP_DIR}/roberta_model.pt")
    ...
    # Evaluate
    evaluate_model(y_train_true, y_train_probs, chosen_threshold, "RoBERTa (Train - Diagnostic)", len(y_train_true), train_inf_time)
    evaluate_model(y_val_true, y_val_probs, chosen_threshold, "RoBERTa (Validation - Tuned)", len(y_val_true), val_inf_time)
    test_metrics = evaluate_model(y_test_true, y_test_probs, chosen_threshold, "RoBERTa (Test - Final Evaluation)", len(y_test_true), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  RoBERTa")
