...
EXP_DIR = "experiments/svm"
MODEL_PATH = f"{EXP_DIR}/svm_model.joblib"
...
def load_data(path):
    df = pd.read_csv(path)
    # Fill missing prompts with empty string
    df["prompt"] = df["prompt"].fillna("").astype(str)
    return df["prompt"].values, df["label"].values

def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - SVM EXPERIMENT")
    print("=" * 60)
    ...
    # 3. Model Training
    print("\nTraining SVM model (SVC with probability=True)...")
    clf = SVC(probability=True, random_state=42, C=1.0, kernel='linear')
    ...
    # Evaluate all splits
    train_metrics = evaluate_model(y_train, y_train_probs, chosen_threshold, "SVM (Train - Diagnostic)", len(y_train), train_inf_time)
    val_metrics = evaluate_model(y_val, y_val_probs, chosen_threshold, "SVM (Validation - Tuned)", len(y_val), val_inf_time)
    test_metrics = evaluate_model(y_test, y_test_probs, chosen_threshold, "SVM (Test - Final Evaluation)", len(y_test), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  SVM")
    print(f"Configuration:              SVC(kernel='linear', C=1.0, probability=True), threshold={chosen_threshold:.2f}")
