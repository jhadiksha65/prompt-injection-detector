...
EXP_DIR = "experiments/xgboost"
MODEL_PATH = f"{EXP_DIR}/xgb_model.joblib"
...
def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - XGBOOST EXPERIMENT")
    print("=" * 60)
    ...
    # XGBoost handles float32 sparse/dense matrix
    ...
    # Evaluate all splits
    train_metrics = evaluate_model(y_train, y_train_probs, chosen_threshold, "XGBoost (Train - Diagnostic)", len(y_train), train_inf_time)
    val_metrics = evaluate_model(y_val, y_val_probs, chosen_threshold, "XGBoost (Validation - Tuned)", len(y_val), val_inf_time)
    test_metrics = evaluate_model(y_test, y_test_probs, chosen_threshold, "XGBoost (Test - Final Evaluation)", len(y_test), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  XGBoost")
    print(f"Configuration:              XGBClassifier(random_state=42), threshold={chosen_threshold:.2f}")
