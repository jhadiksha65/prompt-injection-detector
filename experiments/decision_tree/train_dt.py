...
EXP_DIR = "experiments/decision_tree"
MODEL_PATH = f"{EXP_DIR}/dt_model.joblib"
...
def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - DECISION TREE EXPERIMENT")
    print("=" * 60)
    ...
    print(f"Decision Tree training time: {train_time:.2f} seconds")
    ...
    # Evaluate all splits
    train_metrics = evaluate_model(y_train, y_train_probs, chosen_threshold, "Decision Tree (Train - Diagnostic)", len(y_train), train_inf_time)
    val_metrics = evaluate_model(y_val, y_val_probs, chosen_threshold, "Decision Tree (Validation - Tuned)", len(y_val), val_inf_time)
    test_metrics = evaluate_model(y_test, y_test_probs, chosen_threshold, "Decision Tree (Test - Final Evaluation)", len(y_test), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  Decision Tree")
    print(f"Configuration:              DecisionTreeClassifier(random_state=42), threshold={chosen_threshold:.2f}")
