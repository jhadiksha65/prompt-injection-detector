...
EXP_DIR = "experiments/deberta"
MODEL_CHECKPOINT = "microsoft/deberta-v3-base"
MAX_LEN = 128
BATCH_SIZE = 32
LR = 2e-5
EPOCHS = 3
# Force CPU for DeBERTa to prevent MPS-related NaN losses
...
def main():
    print("=" * 60)
    print("PROMPT INJECTION DETECTOR - DEBERTA EXPERIMENT")
    print("=" * 60)
    ...
    print(f"DeBERTa training completed in {train_time:.2f} seconds")
    
    # Restore best checkpoint
    model.load_state_dict({k: v.to(DEVICE) for k, v in best_model_state.items()})
    
    # Save the model
    torch.save(best_model_state, f"{EXP_DIR}/deberta_model.pt")
    ...
    # Evaluate
    evaluate_model(y_train_true, y_train_probs, chosen_threshold, "DeBERTa (Train - Diagnostic)", len(y_train_true), train_inf_time)
    evaluate_model(y_val_true, y_val_probs, chosen_threshold, "DeBERTa (Validation - Tuned)", len(y_val_true), val_inf_time)
    test_metrics = evaluate_model(y_test_true, y_test_probs, chosen_threshold, "DeBERTa (Test - Final Evaluation)", len(y_test_true), test_inf_time)
    
    print("\n" + "=" * 60)
    print("FINAL SUMMARY SCREENSHOT CAPTURE READY")
    print("=" * 60)
    print("Model Name:                  DeBERTa")
